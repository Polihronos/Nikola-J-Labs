...
class Aggregator:      
    ...
    def _aggregate(self, orders, ascending=True):
        # Group by price and aggregate all volumes for each price using Decimal
        price_volumes = {}
        for price, volume in orders:
            # Convert to Decimal using str() to avoid float precision issues
            price_dec = Decimal(str(price))
            volume_dec = Decimal(str(volume))
            
            if price_dec in price_volumes:
                price_volumes[price_dec] += volume_dec
            else:
                price_volumes[price_dec] = volume_dec
        
        # Sort values based on standard financial order book rules
        sorted_prices = sorted(price_volumes.keys(), reverse=not ascending)
        
        # Convert back to list of lists (convert Decimal back to float for compatibility)
        result = [[float(price), float(price_volumes[price])] for price in sorted_prices]
        ...
        for entity_id, content in data.items():
            ...
            if 'bid' in content:
                for order in content['bid']: 
                    total = total + Decimal(str(order[1]))
    ...
    def volume_side_test(self, aggregated_data):
        ...
        for exchanges in self.data.values():
            for book in exchanges.values():
                for order in book['bid']: raw_bids += Decimal(str(order[1]))
                for order in book['ask']: raw_asks += Decimal(str(order[1]))
